...
func NewAdminSessionsExportHandler(service *SessionService) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = writer.Write([]string{
			"session_id", "parent_session_id", "client_session_id", "created_at", "actor", "agent", "routed_agent", "status",
		...
		for _, s := range summaries {
			u := s.UsageSummary
			_ = writer.Write([]string{
				s.SessionID, s.ParentSessionID, s.ClientSessionID, s.CreatedAt.UTC().Format(time.RFC3339), s.ActorSubject, s.Agent, s.RoutedAgent, s.Status,
